...
import { X, FileJson, Upload, ClipboardPaste } from "lucide-react";
...
export const ImportCardsModal = ({
    ...
    const { execute, isExecuting } = useAction(importCards, {
        ...
        onError: (error) => {
            addToast("Failed to import cards. Check JSON format.", "error");
        }
    });
    ...
    const onImport = () => {
        ...
        execute({ boardId, listId, cardsJson: jsonText });
    };
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-x-3 pt-2">
                        ...
                        >
                            {isExecuting ? "Importing..." : <><ClipboardPaste className="h-4 w-4" /> Import Card(s)</>}
                        </button>
                    </div>
                </div>
            </div>
